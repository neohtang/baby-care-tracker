/**
 * StatisticsDashboardService - 仪表盘与时间线统计
 * 负责首页仪表盘数据聚合、各模块按日统计、时间线事件
 * 从 statistics.ts 拆分而来（Phase 1.3b）
 */

import { feedingStorage, sleepStorage, diaperStorage, healthStorage } from './storage';
...
class StatisticsDashboardService {
  ...
  getTodayFeedingSummary(babyId: string): FeedingDailySummary {
    ...
    const records = feedingStorage
      .getAll()
      .filter((r: FeedingRecord) => r.babyId === babyId && r.startTime.substring(0, 10) === today);
  ...
  getTodaySleepSummary(babyId: string): SleepDailySummary {
    const today = getToday();
    const records = sleepStorage
      .getAll()
      .filter((r: SleepRecord) => r.babyId === babyId && r.startTime.substring(0, 10) === today);
  ...
  getTodayDiaperSummary(babyId: string): DiaperDailySummary {
    const today = getToday();
    const records = diaperStorage
      .getAll()
      .filter((r: DiaperRecord) => r.babyId === babyId && r.time.substring(0, 10) === today);
  ...
  getLatestTemperature(babyId: string): {
    ...
    const records = healthStorage
      .getAll()
      .filter(
        (r: HealthRecord) =>
          r.babyId === babyId && r.recordType === 'temperature' && r.temperature !== undefined,
      );
    ...
    // 按时间倒序，取最近一条
    records.sort(
      (a: HealthRecord, b: HealthRecord) => new Date(b.time).getTime() - new Date(a.time).getTime(),
    );
  ...
  getTodayTimeline(babyId: string): TimelineEvent[] {
    ...
    // 喂养记录
    const feedingRecords = feedingStorage
      .getAll()
      .filter((r: FeedingRecord) => r.babyId === babyId && r.startTime.substring(0, 10) === today);
    ...
    // 睡眠记录
    const sleepRecords = sleepStorage
      .getAll()
      .filter((r: SleepRecord) => r.babyId === babyId && r.startTime.substring(0, 10) === today);
    ...
    // 排便记录
    const diaperRecords = diaperStorage
      .getAll()
      .filter((r: DiaperRecord) => r.babyId === babyId && r.time.substring(0, 10) === today);
    ...
    // 健康记录
    const healthRecords = healthStorage
      .getAll()
      .filter((r: HealthRecord) => r.babyId === babyId && r.time.substring(0, 10) === today);
    healthRecords.forEach((r: HealthRecord) => {
      ...
      if (r.recordType === 'temperature') {
        ...
        if (
          r.temperatureLevel === 'mild_fever' ||
          r.temperatureLevel === 'moderate_fever' ||
          r.temperatureLevel === 'high_fever'
        ) {
...
export const statisticsDashboardService = new StatisticsDashboardService();
export default statisticsDashboardService;
